...
void SceneSerializer::SerializeYAML(std::string_view sceneName)
{
	...
	SL_LOG_INFO("Serialize scene data file: \"{}\"", finalPath);
	...
	if (fileStream.fail())
	{
		SL_LOG_ERROR("Serialize scene failed: \"{}\"", finalPath);
		return;
	}
...
bool SceneSerializer::DeserializeYAML(std::string_view sceneName)
{
	...
	if (fileStream.fail())
	{
		SL_LOG_ERROR("Can not open scene data file: \"{}\"", finalPath);
		return false;
	}

	SL_LOG_INFO("Deserialize scene data file: \"{}\"", finalPath);
	...
	if (!data["Scene"])
	{
		SL_LOG_ERROR("Unknown scene data file: \"{}\"", finalPath);
		return false;
	}
